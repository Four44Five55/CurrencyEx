$(document).ready(function() {
    const host = window.location.origin + "/currency-exchange-app-1.0-SNAPSHOT";
    ...
    $("#convert").submit(function(e) {
        ...
        $.ajax({
            ...
            // data: "$("#add-exchange-rate").serialize()",
            success: function(data) {
                const value = Number(data.convertedAmount).toFixed(2);
                $("#convert-converted-amount").val(value);
            },
            error: function(jqXHR, textStatus, errorThrown) {
                const error = JSON.parse(jqXHR.responseText);
                const toast = $('#api-error-toast');
